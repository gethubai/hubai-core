...
import { TriggerEvent } from '@/common/dom';
  ...
  return (
    ...
    >
      <div className={menuContentClassName}>
        ...
        <span className={labelClassName}>{render ? render(props) : name}</span>
        <Icon className={indicatorClassName} type={`chevron-${chevronType}`} />
      </div>
      {subMenuContent}
    </li>
  );
