...
import { em2Px } from '@/common/css';
import { getRelativePosition, triggerEvent } from '@/common/dom';
...
const setPositionForSubMenu = (
  ...
  if (isAlignHorizontal) pos.y += domRect.height;
  else {
    pos.x += domRect.width;
...
function MenuComp(props: React.PropsWithChildren<IMenuProps>, ref) {
  ...
  const initialMenuStyle = () => {
    ...
    menuRef.current?.querySelectorAll(`li.${activeClassName}`).forEach((li) => {
