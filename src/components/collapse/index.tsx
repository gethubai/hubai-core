...
import { HTMLElementProps, UniqueId } from '@/common/types';
...
export function Collapse({
  ...
  const renderPanels = (data: ICollapseItem, render?: RenderFunctionProps) => {
    if (render) {
      return render(data);
    }
    return null;
    ...
    if (activeLength) {
      ...
      const wipSizes = data.map((pane, index) => {
        ...
        if (typeof adjustedSize.current[index] !== 'undefined') {
          restHeight -= adjustedSize.current[index];
  ...
  const { nextSashes, allowResize } = useMemo(
    function () {
      ...
      for (let i = 1; i < activePanelKeys.length; i++) {
        const prevPaneActive: boolean = !isUndefined(activePanelKeys[i - 1]);
            ...
            <Pane key={panel.id} minSize={HEADER_HEIGTH}>
              ...
              >
                ...
                >
                  ...
                  <span className={collapseTitleClassName}>{panel.name}</span>
