...
function TreeView({
  ...
  const handleDragEnter = debounce(
    (e: React.DragEvent<HTMLDivElement>, node: ITreeNodeItemProps) => {
      ...
      if (!node.isLeaf && !isSelfNode && !isExpand && !canLoadData(uuid)) {
        handleExpandKey(uuid, node);
      }
    },
    300
  ...
  const renderTreeNode = (data: ITreeNodeItemProps[], indent: number) => {
    return data.map((item, index) => {
      ...
      const title = renderTitle?.(item, index, !!item.isLeaf) || item.name;
      ...
      const currentNode = (
        <TreeNode
          ...
          className={classNames(
            ...
            isExpand ? expandTreeNodeClassName : unexpandTreeNodeClassName
          )}
          renderIcon={() =>
            renderIcon(IconComponent, !!item.isLeaf, isExpand, isLoading)
          }
          renderTitle={() => title}
