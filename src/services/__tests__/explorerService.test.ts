import { ExplorerEvent, IExplorerPanelItem } from '@/model';
import 'reflect-metadata';
import { expectFnCalled, expectLoggerErrorToBeCalled } from '@test/utils';
import { container } from 'tsyringe';
import { searchById } from '@/common/utils';
...
describe('Test the Explorer Service', () => {
  ...
  describe('Test the panel data', () => {
    ...
    test('Should support to update the specific panel', () => {
      ...
      const state = explorerService.getState();
      expect(state.data?.find(searchById(panelData.id))).toEqual(panelData);
    ...
    test('Should support to toggle the visibility of the panel', () => {
      ...
      const state = explorerService.getState();
      expect(state.data!.find(searchById(panelData.id))!.hidden).toBeFalsy();
      expect(
        state.headerToolBar?.contextMenu?.find(searchById(panelData.id))?.icon
      ).toBe('check');

      explorerService.togglePanel(panelData.id);

      expect(state.data!.find(searchById(panelData.id))!.hidden).toBeTruthy();
  ...
  describe('Test the actions data in toolbar actions', () => {
    ...
    test('Should support to add a action', () => {
      ...
      expect(explorerService.getAction(actionData.id)).toEqual(actionData);
    });
    ...
    test('Should support to toggle the status of the action', () => {
      ...
      expect(explorerService.getAction(actionData.id)?.icon).toBe('check');
  ...
  test('Should support to subscribe onClick event', () => {
    ...
    explorerService.emit(ExplorerEvent.onClick, new Event('click'), panelData);
