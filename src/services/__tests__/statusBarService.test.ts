...
import { Float, StatusBarEvent } from '@/model/workbench/statusBar';
...
describe('Test StatusBarService', () => {
  ...
  test('Should support to update left first and then right postion', () => {
    ...
    const expected = statusBarService.getStatusBarItem(anotherStatusData.id);
  ...
  test('Should support to remove a item in specific position', () => {
    statusBarService.add(mockStatusData, Float.right);
    expect(statusBarService.getStatusBarItem(mockStatusData.id)).not.toBeNull();
