...
import { ColorThemeMode, ColorScheme } from '@/model/colorTheme';
import {
  BuiltInColorTheme,
  ColorThemeService,
...
describe('The Color Theme Service', () => {
  ...
  test('Should update specific theme', () => {
    ...
    expect(colorThemeService.getThemeById(BuiltInColorTheme.id)?.label).toBe(
      'Dark Test'
    );
  });
  ...
  test('Should support to get colorThemeMode', () => {
    ...
    expect(colorThemeService.getColorThemeMode()).toBe(ColorThemeMode.light);
  ...
  test('Listen to the theme changed event', () => {
    ...
    expect(colorThemeService.getColorTheme()!.id).toEqual(BuiltInColorTheme.id);
  });
});
