...
import { EditorEvent, IEditorTab } from '@/model';
...
describe('Test EditorService', () => {
  ...
  test('Update the tab', () => {
    ...
    let updated = editor.updateTab(Object.assign(mockTab, { name: 'updated' }));
  ...
  test('Set the editor text for a specific group', () => {
    ...
    if (groups) {
      ...
      expect(groups[0].editorInstance?.getModel()?.getValue()).toBe(modifyText);
    }
  });
  ...
  test('Should update editor text via updateTab', () => {
    ...
    expect(setValFn).toHaveBeenCalled();
  ...
  test('Should prevent update editor text if current tab with renderPane', () => {
    ...
    expect(setValFn).not.toHaveBeenCalled();
  });
  ...
  test('Close to right tabs', () => {
    ...
    expect(
      editor.closeToRight({ id: 'unknown' }, editor.getState().current!.id!)
    ).toBeUndefined();
  ...
  test('Close to left tabs', () => {
    ...
    expect(
      editor.closeToLeft({ id: 'unknown' }, editor.getState().current!.id!)
    ).toBeUndefined();
