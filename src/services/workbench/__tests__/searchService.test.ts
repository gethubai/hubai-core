...
import { InfoTypeEnums } from '@/components/search/input';
import { SearchEvent } from '@/model/workbench/search';
import { randomId } from '@/common/utils';
import { modules } from '@/services/builtinService/const';
...
describe('Test StatusBarService', () => {
  ...
  test('Should support to toggle case sensitive', () => {
    ...
    const TEST_RESULT = { ...testValue[0], checked: true };
    ...
    expect(searchBarService.getState().replaceAddons).toEqual([]);
    expect(searchBarService.getState().searchAddons?.[0]).toEqual(TEST_RESULT);
  });

  test('Should support to toggle whole word', () => {
    const testValue = modules.builtInSearchAddons();
    const TEST_RESULT = { ...testValue[1], checked: true };
    ...
    expect(searchBarService.getState().replaceAddons).toEqual([]);
    expect(searchBarService.getState().searchAddons?.[1]).toEqual(TEST_RESULT);
  });

  test('Should support to toggle regex', () => {
    const testValue = modules.builtInSearchAddons();
    const TEST_RESULT = { ...testValue[2], checked: true };
    ...
    expect(searchBarService.getState().replaceAddons).toEqual([]);
    expect(searchBarService.getState().searchAddons?.[2]).toEqual(TEST_RESULT);
  });
  ...
  test('Should support to toggle preserve case', () => {
    ...
    expect(searchBarService.getState().replaceAddons?.[0]).toEqual(TEST_RESULT);
    expect(searchBarService.getState().searchAddons).toEqual([]);
  });
  ...
  test('Should support to trigger replace all', () => {
    ...
    expect(TEST_FN).toHaveBeenCalled();
  });
