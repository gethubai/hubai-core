...
import { IMenuBarItem } from '@/model/workbench/menuBar';
import { Menu, MenuMode, MenuRef, IMenuProps } from '@/components/menu';
...
export function HorizontalView(props: IHorizontalViewProps) {
  ...
  const checkIsRootLiElem = (e: MouseEvent) => {
    ...
    const isRootLiElem =
      !!menuBarElem && menuBarElem.classList.contains(horizontalClassName);
    return isRootLiElem;
  };
    ...
    const handleClickMenuBar = (e: MouseEvent) => {
      ...
      if (isRootLiElem) {
        ...
        const isNormalLiElem =
          liElem && menuBarElem.contains(liElem) && !liElem?.dataset.submenu;
